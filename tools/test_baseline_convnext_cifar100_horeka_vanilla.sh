...
#SBATCH --array=0-29
#SBATCH --job-name=test_baseline_vanilla_convnext_cifar100_horeka
#SBATCH --error=slurm/test_baseline_vanilla_2_convnext_cifar100_horeka_%x_%A_%a.err
#SBATCH --output=slurm/test_baseline_vanilla_2_convnext_cifar100_horeka_%x_%A_%a.out
...
CUDA_VISIBLE_DEVICES=0 python tools/test.py configs/classification/cifar100/mixups/vits/convnext_tiny/convnext_t_vanilla_bs100_ep200.py $CKPT_PATH --work_dir ${SAVE_DIR} --seed ${SEED} --launcher none --sparsity-threshold $SPARSITY_THRESHOLD --sparsity-type percentage --eval-corruptions --sparse-baseline
